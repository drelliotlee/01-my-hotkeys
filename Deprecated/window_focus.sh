...
activate_window() {
    ...
    if [[ "$name" == *"Gemini"* ]]; then
        local info=$(xwininfo -id "$win")
        local abs_y=$(awk '/Absolute upper-left Y:/ {print $4}' <<<"$info")
        local rx ry
        if (( abs_y > 1500 )); then rx=404; ry=2196; else rx=514; ry=2249; fi
        xdotool mousemove --window "$win" "$rx" "$ry"
        sleep 0.05
        xdotool click 1
    else
